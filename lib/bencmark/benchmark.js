...
var Benchmark = /** @class */ (function () {
    ...
    Benchmark.saveMeasurementTimesSeries = function (pathToMeasurements) {
        var _this = this;
        pathToMeasurements !== null && pathToMeasurements !== void 0 ? pathToMeasurements : (pathToMeasurements = './time_series');
        var opsPerSecond = '';
        var dataBuffer = '';
        if (!(0, fs_1.existsSync)(pathToMeasurements)) {
            (0, fs_1.mkdirSync)(pathToMeasurements);
        }
        Benchmark.generatorFunctions.forEach(function (fn) {
            var comparedResult = _this.getFunctionTimeSeriesAndIterationIndices(fn);
            var timeToGeneration = comparedResult.timeToGeneration, iterationIndices = comparedResult.iterationIndices, measurementHash = comparedResult.measurementHash;
            opsPerSecond = timeToGeneration.map(utils_1.calculateOpsPerSecond).join(', ');
            dataBuffer += "".concat((0, utils_1.getFunctionName)(fn), "\n").concat(opsPerSecond, "\n");
        });
        (0, fs_1.writeFileSync)("".concat(pathToMeasurements, "/").concat((0, utils_1.getFunctionName)(Benchmark.generatorFunctions[0]), ".txt"), dataBuffer, { flag: 'w+' });
    };
